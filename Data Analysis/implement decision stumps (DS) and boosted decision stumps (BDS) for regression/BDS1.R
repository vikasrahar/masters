...
if(min_lstat < min_rm ){
  best_value <- array_lstat[which.min(array_rss_lstat)]
  new_attribute <- "lstat"
}else{
  best_value <- array_rm[which.min(array_rss_rm)]
  new_attribute <- "rm"
}
...
#Step 5 ->>> calculate MSE value
rss_test_val <- sum((test_Y - y_hat_value)^2) / length(test_Y) #test RSS
